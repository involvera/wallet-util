...
import { sha256 as SHA256, hmac as HMAC } from '../hashes';
...
export const utils = {
  ...
  // Takes curve order + 64 bits from CSPRN
  // so that modulo bias is neglible, matches FIPS 18
  randomPrivateKey: (): Uint8Array => {
    return utils.hashToPrivateKey(RandomBytes(40));
  },
  ...
  sha256: (...messages: Uint8Array[]) => {
    const sha = SHA256.create()
    for (let msg of messages) sha.update(msg);
    return new Uint8Array(sha.digest());
  },
  
  hmacSha256: (key: Uint8Array, ...messages: Uint8Array[]) => {
    const sha = HMAC.create(SHA256, key)
    for (let msg of messages) sha.update(msg);
    return new Uint8Array(sha.digest());
  },
